...
Liste von Monitoren. Jeder Monitor läuft unabhängig als eigener Hintergrund-Loop und publiziert ein eigenes Set aus
...
Eigene Typen (z.B. `_ipp._udp`, `_workstation._tcp`) können als Strings in `service_types` ergänzt werden — die
Validierung ist bewusst `list(str)`, kein Enum.
...
- `online`: Dienst angekündigt **und** per `avahi-resolve` auflösbar — Sensor `ON`, `state` = `online`
...
Der Check funktioniert nur, wenn Multicast-Pakete zwischen Container und LAN fließen können. Da der Add-on
`host_network: true` nutzt, ist das in der Regel gegeben. Sicherstellen:
...
- `sensor._last_check`). Ab 0.4.0 ist es nur noch **eine** Binary-Sensor-Entity; `state` (Text) und `last_check`
