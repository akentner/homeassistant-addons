...
Every add-on maintains versions across exactly 3 files that must stay in sync. This is enforced by pre-commit hooks (`scripts/validate-versions.sh`):
...
**Never edit versions manually.** Always use `make update-version` or `scripts/update-version.py`. The subpatch suffix in config.yaml resets to `-0` when upstream version changes; increment it (e.g., `-1`, `-2`) for add-on-only fixes.
...
`.upstream.yaml` in each add-on directory configures automated version tracking via GitHub Actions (daily at 6:00 UTC). The `version_pattern: "sync"` in the addon section means the add-on version follows upstream exactly.
