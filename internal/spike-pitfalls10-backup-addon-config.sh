...
green "  ssh ${HOST} sudo ha backups new '${BACKUP_NAME}'"
ssh "${HOST}" sudo ha backups new "${BACKUP_NAME}" 2>&1 || {
...
ssh "${HOST}" sudo ha backups list 2>&1 | grep "${BACKUP_NAME}" || {
    red "  ✗ Backup ${BACKUP_NAME} not visible in 'ha backups list'"
    echo "RESULT: inconclusive"
    exit 1
}
...
green "  rsync -av ${HOST}:/backup/${BACKUP_NAME} /tmp/"
BACKUP_DIR="/tmp/phase9-backup-${RUN_ID}"
mkdir -p "${BACKUP_DIR}"
rsync -av "${HOST}:/backup/${BACKUP_NAME}" /tmp/ 2>&1 | tail -3
green "  tar -xf '/tmp/${BACKUP_NAME}' -C '${BACKUP_DIR}'"
tar -xf "/tmp/${BACKUP_NAME}" -C "${BACKUP_DIR}" 2>&1
