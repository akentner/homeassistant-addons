...
func TestTokenStoreGeneratePersistLoadValidate(t *testing.T) {
	...
	info, err := os.Stat(filepath.Join(dir, "iac-runner-token"))
	if err != nil {
...
func TestWriteInitialTokenCreatesChmod600(t *testing.T) {
	dir := t.TempDir()
	store, err := NewFileTokenStore(dir)
	if err != nil {
		t.Fatalf("NewFileTokenStore: %v", err)
	...
	if path != filepath.Join(dir, "initial-iac-runner-token") {
		t.Errorf("path = %q, want %q", path, filepath.Join(dir, "initial-iac-runner-token"))
	}

	info, err := os.Stat(path)
	if err != nil {
		t.Fatalf("stat initial-iac-runner-token: %v", err)
	}
	if perm := info.Mode().Perm(); perm != 0o600 {
		t.Errorf("initial-iac-runner-token mode = %o, want 0600", perm)
	}

	body, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("read initial-iac-runner-token: %v", err)
	}
	if strings.TrimSpace(string(body)) != token {
		t.Errorf("initial-iac-runner-token body = %q, want %q", string(body), token)
	}
}
...
// TestNewFileTokenStoreCreatesMissingDataDir verifies that
// NewFileTokenStore creates dataDir if it does not already exist
// (mirrors terraform-bridge invariant). Prevents first-start crash
// when no /data volume is mounted.
...
func TestGraceWindowAcceptsPreviousHash(t *testing.T) {
	...
	// Inject a grace entry directly (Rotate is the real path).
	store.mu.Lock()
...
func TestTokenStoreGracePersistsAcrossReload(t *testing.T) {
	...
	graceBytes, err := os.ReadFile(filepath.Join(dir, "iac-runner-token.grace"))
	if err != nil {
	...
	info, err := os.Stat(filepath.Join(dir, "iac-runner-token.grace"))
	if err != nil {
