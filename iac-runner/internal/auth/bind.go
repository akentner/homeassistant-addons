...
// ResolveBindAddress decides which interface IP the iac-runner
// should listen on. The decision tree:
//
...
//
// The function takes the `sysClassNet` root + an `addrFn` parameter so
// tests can inject a stub that returns canned addresses per
// interface name. Production callers (main.go) pass nil for `addrFn`
// to use DefaultAddrFn, which wraps net.InterfaceByName + Addrs.
func ResolveBindAddress(bindAddress string, allowedSubnets []string, sysClassNet string,
	addrFn func(ifaceName string) ([]net.IP, error)) (string, error) {
	if bindAddress == "0.0.0.0" {
		return "", errors.New(
			"auth: bind_address=0.0.0.0 is always refused; use bind_allowed_subnets to enumerate explicit subnets",
		)
	}
	...
	if bindAddress == "auto" || bindAddress == "" {
		ip, err := firstTailscaleIP(sysClassNet, addrFn)
		if err != nil {
	...
	if onTailscaleInterface(ip, sysClassNet, addrFn) {
...
// used when ResolveBindAddress is called with nil addrFn. Tests inject
// a closure that returns canned []net.IP per iface name to avoid
// hitting the real kernel.
...
// the addrFn-supplied address lookup for that interface.
func firstTailscaleIP(sysClassNet string, addrFn func(ifaceName string) ([]net.IP, error)) (string, error) {
...
func onTailscaleInterface(ip net.IP, sysClassNet string, addrFn func(ifaceName string) ([]net.IP, error)) bool {
	matches, err := filepath.Glob(filepath.Join(sysClassNet, "tailscale*"))
	if err != nil {
		return false
	}
