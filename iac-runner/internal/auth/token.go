// Package auth implements the iac-runner bearer-token primitive:
// generation, persistence (atomic, chmod 600), validation
// (constant-time compare against the on-disk SHA-256 hash), and a
// rotation-with-grace flow used by POST /v1/auth/rotate.
//
// AUTHR-02 / AUTHR-03 / AUTHR-04 are satisfied by this package:
//   - AUTHR-02: Generate() uses crypto/rand (32 bytes); plaintext is
//     base64url-encoded with RawURLEncoding so the result is 43 chars,
//     URL-safe, and has no padding that would need escaping.
//   - AUTHR-03: Validate() compares the SHA-256 of the presented token
//     against the on-disk hash via subtle.ConstantTimeCompare.
//   - AUTHR-04: Rotate() writes the previous hash to a
...
//     start, at /data/initial-iac-runner-token (chmod 600, atomic
//     rename), so operators can configure their runner client without
//     the token ever passing through a log stream. Subsequent restarts
//     do NOT re-write this file.
//
// This package is slog-free by invariant (verified by `! grep -RIn
// 'slog\.' iac-runner/internal/auth/`); the auth package never
// emits log records. Callers (main.go, handlers/auth_rotate.go) own
// the audit logging.
package auth
...
// ErrNoToken is returned by Load when /data/iac-runner-token does
// not yet exist (first-start condition). Callers translate this to
...
// 401 with body {"error_code":"unauthorized"}.
...
type TokenStore struct {
	...
	tokenPath string // /data/iac-runner-token
	gracePath string // /data/iac-runner-token.grace
...
func NewFileTokenStore(dataDir string) (*TokenStore, error) {
	...
	s := &TokenStore{
		dataDir:   dataDir,
		tokenPath: filepath.Join(dataDir, "iac-runner-token"),
		gracePath: filepath.Join(dataDir, "iac-runner-token.grace"),
	}
...
// /data/iac-runner-token with chmod 600. The write is atomic: a
...
func (s *TokenStore) Persist(plaintext string) error {
	...
	tmp, err := os.CreateTemp(s.dataDir, ".iac-runner-token.*.tmp")
	if err != nil {
...
// /data/initial-iac-runner-token (chmod 600, atomic rename) on first
// start so operators can configure their runner client without the
// plaintext ever passing through a log stream. The returned path is
// logged (along with the truncated preview emitted by Truncate) in
// the iac_runner.token.issued record. Subsequent restarts do NOT
// call this method — the file is written exactly once. Operators
// SHOULD delete the file after configuring their client so the
// plaintext is not stored on disk any longer than necessary.
func (s *TokenStore) WriteInitialTokenFile(plaintext string) (string, error) {
	path := filepath.Join(s.dataDir, "initial-iac-runner-token")

	tmp, err := os.CreateTemp(s.dataDir, ".initial-iac-runner-token.*.tmp")
	if err != nil {
		return "", fmt.Errorf("auth: create initial-iac-runner-token tmp: %w", err)
	}
	tmpName := tmp.Name()
	defer os.Remove(tmpName) // no-op if rename succeeded
	...
	if _, err := tmp.WriteString(plaintext + "\n"); err != nil {
		tmp.Close()
		return "", fmt.Errorf("auth: write initial-iac-runner-token: %w", err)
	}
	if err := tmp.Sync(); err != nil {
		tmp.Close()
		return "", fmt.Errorf("auth: sync initial-iac-runner-token: %w", err)
	}
	if err := tmp.Close(); err != nil {
		return "", fmt.Errorf("auth: close initial-iac-runner-token: %w", err)
	}
	if err := os.Chmod(tmpName, 0o600); err != nil {
		return "", fmt.Errorf("auth: chmod initial-iac-runner-token: %w", err)
	}
	if err := os.Rename(tmpName, path); err != nil {
		return "", fmt.Errorf("auth: rename initial-iac-runner-token: %w", err)
	}
	return path, nil
}

// Hash returns the on-disk SHA-256 hash (copy), used by handlers to
// compute actor_token_fp. Returns nil if no token has been generated
// yet (first-start, pre-Generate).
...
// hash stored in /data/iac-runner-token.grace. Returns ErrInvalidToken
// otherwise. Comparison is constant-time via subtle.ConstantTimeCompare.
//
// Grace expiry is checked on every call: if expiresAt is past, the
// previous-hash match path is skipped and only the current hash can
// succeed.
...
// GraceWindow is the lifetime of a rotation grace period.
...
// it in the POST /v1/auth/rotate response body exactly once).
...
// first-start emission (iac_runner.token.issued) so the log carries a
...
// plaintext is no longer available (used by main.go's
// iac_runner.token.loaded record and by Rotate when fingerprinting
// the just-replaced token without re-deriving its plaintext). Both
// fingerprints are derivable from the same SHA-256 input, so
// HashFingerprint(hash) == Fingerprint(plaintext) for any plaintext
// hashing to `hash`.
...
// /data/iac-runner-token file (chmod 600, atomic rename), and
// persists the previous hash to /data/iac-runner-token.grace with a
// grace expiry of now + GraceWindow. The returned RotateResult
// carries the new plaintext (caller surfaces once) and three
// fingerprints for the audit record. After Rotate returns, Validate
// accepts BOTH the old and new tokens until GraceExpiresAt.
//
...
func (s *TokenStore) Rotate() (RotateResult, error) {
	...
	gtmp, err := os.CreateTemp(s.dataDir, ".iac-runner-token.grace.*.tmp")
	if err != nil {
