...
A Go HTTP service on port 8125 (distinct from `terraform-bridge`'s 8124) that clones a Git repo (e.g. `homelab-infra`),
runs `tofu plan`/`apply` against Tailscale-reachable homelab servers, and persists state in Cloudflare R2 (default), any
S3-compatible backend, or a local file. Manual REST trigger only (`POST /v1/plan`, `POST /v1/apply` →
`GET /v1/runs/{id}`); concurrent applies on the same repo are serialized by an in-process mutex.

**Features:**

- Bearer-auth on `0.0.0.0:8125` with Tailscale-bind gate (`bind_address: 0.0.0.0` always refused) — no `hassio_api`, no
  `SUPERVISOR_TOKEN`
...
# Bump an add-on's version (NEVER edit versions manually — also creates and pushes the <addon>/v<version> git tag)
