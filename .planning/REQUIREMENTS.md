...
- [x] **MER-02**: Dockerfile uses multi-stage build: `oven/bun:1` for TypeScript compilation,
      `ghcr.io/home-assistant/amd64-base` as runtime base with `nodejs` and `npm` installed via `apk`
- [x] **MER-03**: Source is fetched from the GitHub Release archive (`rynfar/meridian`) at Docker build time — no
...
- [x] **MER-08**: Version tracked via 3-file scheme; `.upstream.yaml` watches `rynfar/meridian` for new releases
