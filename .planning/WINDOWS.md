...
> Cross-phase defect register. With `workflow.windows_enforce` enabled, `/gsd-ship` blocks while `open_count > 0`. Waive
> with `gsd-tools windows waive <id> "<reason>"` (reason required). Mark fixed with `gsd-tools windows fixed <id>`.
